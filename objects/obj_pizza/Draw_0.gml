...
for (var _i = 0; _i < array_length(ketchup_trails); _i++)
{
	...
	draw_sprite_general(spr_ketchup_trail, 0, 0, ketchup_trails[_i].sprite_offset,
...
{
	var _rot = arctan2(current_trail.y2 - current_trail.y1,
					   current_trail.x2 - current_trail.x1);
	var _height = sqrt(sqr(current_trail.y2 - current_trail.y1) +
						   sqr(current_trail.x2 - current_trail.x1));
	draw_sprite_general(spr_ketchup_trail, 0, 0, current_trail.sprite_offset,
						 sprite_get_width(spr_ketchup_trail), _height,
						 current_trail.x1 + x, current_trail.y1 + y, 1, 1,
						 _rot, c_white, c_white, c_white, c_white, 1);
}
